...
var AddContactView = /*#__PURE__*/function () {
  //acess the modal like open and close it
...
var ContactContainerView = /*#__PURE__*/function () {
  ...
  _createClass(ContactContainerView, [{
    ...
    value: function pushContactIntoContainer(contact) {
      //recently added contact are put in top
...
function _generateHTMLString2() {
  ...
  if (Array.isArray(data)) {
    //for each array we want to create a contact card
    data.forEach(function (contact) {
      html += _classPrivateMethodGet(_this2, _generatCardContactHTML, _generatCardContactHTML2).call(_this2, contact); //add html elements by call another fn below
    });
    return html;
  }
}
...
var controlContactDelete = function controlContactDelete(id) {
  var list = getContactsFromLS(); //get contact
  ...
  if (Array.isArray(list)) {
    ...
    _ContactContainerView.default.render(list); //rerender it
...
var controlAddContact = function controlAddContact(event) {
  ...
  var Name = _AddContactView.default.Name,
      ...
      Phone = _AddContactView.default.Phone; //extract value from addcontactview
...
var handleSearch = function handleSearch(event) {
  ...
  location.hash = "#q=".concat(query); //it apply in hash
};

var search = function search(query) {
  //to make search opertion
  var list = getContactsFromLS();
  var results = []; //to make firstofall results are empty
